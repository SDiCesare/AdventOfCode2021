...
void algorithm(std::vector<std::string> *start, std::vector<std::string> previous, std::map<std::string, std::vector<std::string> *> *map, bool hasMultiple = false)
{
    std::string current = getKey(*map, start);
    if (current != "start" && current != "end" && !std::isupper(current[0]))
    {
        previous.push_back(current);
    }
    if (std::count(previous.begin(), previous.end(), current) == 2)
    {
        hasMultiple = true;
    }
    for (int i = 0; i < start->size(); i++)
    {
        ...
        if (hasMultiple)
        {
            if (std::count(previous.begin(), previous.end(), var) < 1)
            {
                algorithm(map->at(var), previous, map, hasMultiple);
            }
        }
        else
        {
